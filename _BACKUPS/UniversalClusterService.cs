...
namespace JSE_RevitAddin_MEP_OPENINGS.Services
{
    ...
    public class UniversalClusterService
    {
        ...
        // Helper struct for grouping key (extracted to SleeveGroupKey.cs for Phase 8 sharing)
        ...
        /// </summary>
        // Phase 6,7,8 injected services
        private readonly Clustering.Rotation.IClusterRotationService _rotationService;
        private readonly Clustering.Cleanup.IClusterCleanupService _cleanupService;
        private readonly Clustering.Algorithm.IClusterAlgorithmService _algorithmService;
        private readonly Clustering.Data.IClusterDataService _dataService;
        private readonly Clustering.Timeout.IClusterTimeoutService _timeoutService;

        public UniversalClusterService(FlagManager flagManager = null, FilterManagementService filterService = null, Clustering.Rotation.IClusterRotationService rotationService = null, Clustering.Cleanup.IClusterCleanupService cleanupService = null, Clustering.Algorithm.IClusterAlgorithmService algorithmService = null, Clustering.Data.IClusterDataService dataService = null, Clustering.Timeout.IClusterTimeoutService timeoutService = null)
        {
            ...
            _rotationService = rotationService; // Optional until Phase 6
            _cleanupService = cleanupService; // Optional until Phase 7
            _algorithmService = algorithmService; // Optional until Phase 8
            _dataService = dataService; // Optional until Phase 9
            _timeoutService = timeoutService; // Optional until Phase 10
        }
            ...
            int? filterId = null)
        {
            ...
            try
            {
                ...
                if (!DeploymentConfiguration.DeploymentMode)
                {
                    SafeFileLogger.SafeAppendText("orchestrator_debug.log", $"[{DateTime.Now:HH:mm:ss}] 🔥 CLUSTER SERVICE WORKING WITH XML FILE: {xmlFilePath ?? "NULL"}\n");
                    SafeFileLogger.SafeAppendText("orchestrator_debug.log", $"[{DateTime.Now:HH:mm:ss}] 🔥 CLUSTER SERVICE FILTER NAME: {filterName ?? "NULL"}\n");
                }
                
                // ⚠️ CRITICAL: Reset cluster flags for deleted cluster sleeves
                if (_cleanupService != null)
                    _cleanupService.ResetClusterFlagsForDeletedSleeves(doc, xmlFilePath);
                else
                ...
                {
                    _loadedClashZonesCache = _dataService?.LoadClashZonesFromRegularXml(xmlFilePath, targetCategory, doc)
                                           ?? LoadClashZonesFromRegularXml(xmlFilePath, targetCategory, doc);
                    if (!DeploymentConfiguration.DeploymentMode)
                ...
                if (_dataService != null)
                    _dataService.LoadClashZoneCacheFromLoadedClashZones(allClashZonesFromXml, targetCategory);
                else
                ...
                if (!DeploymentConfiguration.DeploymentMode)
                {
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"JoinOpeningsDistance: {toleranceMm}mm\n");
                }
                                // ✅ DEPLOYMENT MODE: Skip file writes
                if (!DeploymentConfiguration.DeploymentMode)
                {
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"Target category: {targetCategory ?? "ALL"}\n");
                }
                ...
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"✅ DATABASE-FIRST CLUSTERING: Loading clash zones from database/XML (no Revit API calls)\n");
                ...
                { 
                    if (!DeploymentConfiguration.DeploymentMode)
                    {
                        SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING-START] Loaded {allClashZonesFromXml.Count} total clash zones from XML\n");
                        SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING-START] With SleeveInstanceId>0: {withSleeveId}, Not IsClusterResolved: {notClusterResolved}\n");
                    }
                } 
                catch { }
                ...
                if (!DeploymentConfiguration.DeploymentMode)
                {
                    ...
                    SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING-FILTER] Total clash zones: {totalClashZones}\n");
                    SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING-FILTER] With SleeveInstanceId>0: {withSleeveId}\n");
                    SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING-FILTER] Not IsClusterResolved: {notClusterResolved}\n");
                    SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING-FILTER] Category filtered ({targetCategory ?? "ALL"}): {categoryFiltered}\n");
                    SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING-FILTER] By HostType: {string.Join(", ", byHostType.Select(kvp => $"{kvp.Key}={kvp.Value}"))}\n");
                }
                ...
                {
                    // Reuse placementDebugPath from line 207
                    SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING-FILTER] Before bbox filter: {beforeBboxFilter}\n");
                    SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING-FILTER] With null bbox: {withNullBbox} (by host: {string.Join(", ", byHostTypeNullBbox.Select(kvp => $"{kvp.Key}={kvp.Value}"))})\n");
                    SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING-FILTER] After bbox filter: {rawSleeves.Count} (by host: {string.Join(", ", rawSleeves.GroupBy(s => s.HostType).Select(g => $"{g.Key}={g.Count()}"))})\n");
                }
                ...
                if (zeroBboxSleeves.Count > 0)
                {
                    try 
                    { 
                        SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING] ⚠️ Found {zeroBboxSleeves.Count} sleeves with zero bounding boxes (filtered out): [{string.Join(", ", zeroBboxSleeves.Take(10))}...]\n"); 
                    } 
                    catch { }
                }
                ...
                { 
                    if (!DeploymentConfiguration.DeploymentMode)
                    {
                        SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING] Found {rawSleeves.Count} sleeves ready for clustering after filtering:\n");
                    }
                    ...
                    SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING] First 5: {string.Join("\n", firstFew)}\n");
                } 
                catch { }
                ...
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"Processing {rawSleeves.Count} sleeves (category-filtered for {targetCategory ?? "ALL"})\n");
                
                // ⚠️ CRITICAL: Log flag states of sleeves being processed for clustering (AFTER filtering)
                if (!DeploymentConfiguration.DeploymentMode)
                {
                    SafeFileLogger.SafeAppendText("flag_state_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTER-START] Processing {rawSleeves.Count} sleeves for clustering (filtered for {targetCategory ?? "ALL"})\n");
                }
                ...
                var sleeveGroups = rawSleeves.GroupBy(sleeve => {
                    ...
                    if (!DeploymentConfiguration.DeploymentMode)
                    {
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"Sleeve {sleeve.SleeveInstanceId}: hostType={hostType}, systemType={systemType}, orientation={effectiveOrientation}\n");
                    }
                ...
                foreach (var g in sleeveGroups)
                {
                    ...
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"Group: hostType={g.Key.hostType}, systemType={g.Key.systemType}, orientation={g.Key.orientation}, count={list.Count}, sampleIds={string.Join(",", ids)}\n");
                }

                // ⚠️ CRITICAL: Add timeout protection for clustering operation (Phase 10 extracted service)
                if (_timeoutService != null)
                ...
                // Form clusters using spatial hashing (Phase 8 extracted service)
                var clustersByGroup = _algorithmService != null 
                    ? _algorithmService.FormClusters(sleeveGroups, toleranceDist, doc, enableParallel: true)
                    : FormClusters(sleeveGroups, toleranceDist); // legacy fallback
                
                // ⚠️ CRITICAL: Check timeout after FormClusters
                bool isTimedOut = _timeoutService?.IsTimedOut() ?? (clusteringTimer.ElapsedMilliseconds > MAX_CLUSTERING_TIME_MS);
                if (isTimedOut)
                {
                    if (!DeploymentConfiguration.DeploymentMode)
                    {
                        DebugLogger.Error($"[UniversalClusterService] ⏱ TIMEOUT: Clustering exceeded {(_timeoutService?.TimeoutLimitMs ?? MAX_CLUSTERING_TIME_MS) / 1000} second limit during FormClusters");
                    }
                    if (_timeoutService != null)
                ...
                { 
                    if (!DeploymentConfiguration.DeploymentMode)
                    {
                        SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING] FormClusters returned {totalClusters} total clusters ({clustersWithMultipleSleeves} with >1 sleeve, {individualSleevesSkipped} individual sleeves skipped) with {totalSleevesInClusters} total sleeves\n");
                    ...
                    foreach (var groupEntry in clustersByGroup)
                    {
                        ...
                        if (!DeploymentConfiguration.DeploymentMode)
                        {
                            SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING] Group {groupEntry.Key.hostType}_{groupEntry.Key.systemType}_{groupEntry.Key.orientation}: {groupEntry.Value.Count} clusters ({groupClustersWithMultiple} with >1 sleeve, {groupEntry.Value.Count - groupClustersWithMultiple} individual)\n");
                        }
                    }
                } 
                catch { }
                ...
                foreach (var groupEntry in clustersByGroup)
                {
                    // ⚠️ CRITICAL: Check timeout every 5 clusters (Phase 10 extracted service)
                    clusterProcessedCount++;
                    if (clusterProcessedCount % 5 == 0)
                    {
                        bool timedOut = _timeoutService?.IsTimedOut() ?? (clusteringTimer.ElapsedMilliseconds > MAX_CLUSTERING_TIME_MS);
                        if (timedOut)
                        {
                            if (!DeploymentConfiguration.DeploymentMode)
                            {
                                DebugLogger.Error($"[UniversalClusterService] ⏱ TIMEOUT: Clustering exceeded {(_timeoutService?.TimeoutLimitMs ?? MAX_CLUSTERING_TIME_MS) / 1000} second limit after processing {clusterProcessedCount} clusters");
                            }
                            if (_timeoutService != null)
                                _timeoutService.ShowTimeoutWarning($"after processing {clusterProcessedCount} clusters", clusterProcessedCount, totalClusters);
                            else
                                System.Windows.Forms.MessageBox.Show(
                    ...
                    if (cluster.Count <= 1) continue; // Skip individual sleeves
                        ...
                        try
                        {
                            ...
                            { 
                                if (!DeploymentConfiguration.DeploymentMode)
                                {
                                    SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING] Processing cluster with {cluster.Count} sleeves: [{clusterSleeveIds}]\n");
                                }
                            } 
                            catch { }
                            ...
                            if (IsClusterAlreadyExists(cluster, groupKey))
                            {
                                ...
                                { 
                                    if (!DeploymentConfiguration.DeploymentMode)
                                    {
                                        SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING] ⚠️ SKIP: Cluster already exists for sleeves [{clusterSleeveIds}]\n");
                                    }
                                } 
                                catch { }
                                continue;
                            ...
                            { 
                                if (!DeploymentConfiguration.DeploymentMode)
                                {
                                    SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING] ✓ CALLING PlaceClusterSleeve for [{clusterSleeveIds}]\n");
                                }
                            } 
                            catch { }
                            ...
                            try
                            {
                                if (!DeploymentConfiguration.DeploymentMode)
                                {
                                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[CLUSTER-PLACEMENT-CALL] About to call PlaceClusterSleeve for cluster with {cluster.Count} sleeves\n");
                                }
                                ...
                                if (!DeploymentConfiguration.DeploymentMode)
                                {
                                    ...
                                    {
                                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[CLUSTER-ID-CAPTURE] ✅ Using captured ID from PlaceClusterSleeve output parameter: {capturedClusterSleeveId.Value} (category={targetCategory})\n");
                                    }
                                    else
                                    {
                                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[CLUSTER-ID-CAPTURE] ⚠️ WARNING: No captured ID from PlaceClusterSleeve (category={targetCategory}, placed1={placed1}, deleted1={deleted1})\n");
                                    }
                                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[CLUSTER-PLACEMENT-CALL] ✅ PlaceClusterSleeve call completed (no exception)\n");
                                }
                            }
                            catch (Exception placeEx)
                            {
                                if (!DeploymentConfiguration.DeploymentMode)
                                {
                                    ...
                                    SafeFileLogger.SafeAppendText("cluster_debug.log", errorLog);
                                }
                            ...
                            try
                            {
                                if (!DeploymentConfiguration.DeploymentMode)
                                {
                                    ...
                                    SafeFileLogger.SafeAppendText("cluster_debug.log", returnLog);
                                    ...
                                    if (placed1 == 0)
                                    {
                                        ...
                                        SafeFileLogger.SafeAppendText("cluster_debug.log", warningLog);
                                    }
                            ...
                            { 
                                if (!DeploymentConfiguration.DeploymentMode)
                                {
                                    ...
                                    string clusterSleeveIdStr = capturedClusterSleeveId.HasValue 
                                        ...
                                        : (placedClusterSleeve != null ? "INVALID_ELEMENT" : "NULL");
                                    SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING] PlaceClusterSleeve returned: placed={placed1}, deleted={deleted1}, clusterSleeve={clusterSleeveIdStr}\n");
                                }
                            } 
                            catch { }
                            placedCount += placed1;
                            ...
                            if (!DeploymentConfiguration.DeploymentMode)
                            {
                                ...
                                SafeFileLogger.SafeAppendText("cluster_debug.log", beforeCheckMsg);
                            }
                            ...
                            if (placedClusterSleeve != null && capturedClusterSleeveId.HasValue)
                            {
                                ...
                                if (!DeploymentConfiguration.DeploymentMode)
                                {
                                    ...
                                    SafeFileLogger.SafeAppendText("cluster_debug.log", trackingMsg);
                                }
                            ...
                            else
                            {
                                if (!DeploymentConfiguration.DeploymentMode)
                                {
                                    ...
                                    SafeFileLogger.SafeAppendText("cluster_debug.log", errorMsg);
                                }
                            }
                ...
                if (placedClusterSleevesOut != null)
                {
                                        ...
                                        if (!DeploymentConfiguration.DeploymentMode)
                    {
                        ...
                        SafeFileLogger.SafeAppendText("cluster_debug.log", returnMsg);
                    }
                }
        ...
        private void MarkClashZonesAsClusterResolvedWithSleeveId(List<dynamic> cluster, ElementId clusterSleeveId, string xmlFilePath = null, BoundingBoxXYZ clusterBbox = null, (double minX, double minY, double minZ, double maxX, double maxY, double maxZ)? rotatedBbox = null)
        {
            try
            {
                if (!DeploymentConfiguration.DeploymentMode)
                {
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[MarkClusterResolved] ✅ DATABASE-FIRST: Updating cluster flags in database for cluster sleeve {clusterSleeveId.IntegerValue}\n");
                }
                ...
                if (updatedClashZones.Count > 0)
                {
                    ...
                    if (Directory.Exists(filtersDirectory))
                    {
                        ...
                        if (!DeploymentConfiguration.DeploymentMode)
                        {
                            SafeFileLogger.SafeAppendText("cluster_debug.log", $"[MarkClusterResolved] Updating {xmlFiles.Length} XML file(s) after database update: {(string.IsNullOrEmpty(xmlFilePath) ? "ALL" : Path.GetFileName(xmlFilePath))}\n");
                        }
        ...
        /// </summary>
        [Obsolete("Use IClusterCleanupService.ResetClusterFlagsForDeletedSleeves")] // Phase 7
        private void ResetClusterFlagsForDeletedSleeves(Document doc, string xmlFilePath = null)
        {
            try
            {
                if (!DeploymentConfiguration.DeploymentMode)
                {
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[ResetFlags] Processing database for deleted cluster sleeves\n");
                }
        ...
        [Obsolete("Use IClusterAlgorithmService.FormClusters")] // Phase 8
            ...
            double toleranceDist)
        {
            ...
            if (!DeploymentConfiguration.DeploymentMode)
            {
                ...
                {
                    var placementDebugPath = SafeFileLogger.GetLogFilePath("placement_debug.log");
                    SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING] 🚀 MULTI-THREADING: Processing {groupsList.Count} groups in parallel (CPU cores: {Environment.ProcessorCount})\n");
                }
                catch { }
            }
            ...
            try
            {
                ...
                { 
                        ...
                        if (clusters.Count > 0)
                        {
                            ...
                            if (!DeploymentConfiguration.DeploymentMode)
                            {
                                ...
                                lock (_logLock)
                                {
                                    foreach (var cluster in clusters)
                                    {
                                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"  - Cluster with {cluster.Count} sleeves: {string.Join(", ", cluster.Select(s => s.SleeveInstanceId))}\n");
                                    }
                                }
                            }
                        }
                    ...
                    if (!DeploymentConfiguration.DeploymentMode)
                    {
                        ...
                        {
                            var placementDebugPath = SafeFileLogger.GetLogFilePath("placement_debug.log");
                            SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING] ✅ MULTI-THREADING: Completed processing {groupsList.Count} groups\n");
                        }
                        catch { }
                    }
                }
                ...
                while (foundNewNeighbors)
                {
                    ...
                    foreach (var clusterSleeve in cluster.ToList())
                    {
                        foreach (var otherSleeve in xmlSleeves)
                        {
                            ...
                            {
                                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ========== CLUSTERING PATH SELECTION ==========\n");
                                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Checking: Sleeve {clusterSleeve.SleeveInstanceId} vs Sleeve {otherSleeve.SleeveInstanceId}\n");
                                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ✅ SKIPPING ROTATED CLUSTERING: Round pipe/duct detected - using axis-aligned clustering\n");
                            }
                            ...
                            if (!isRoundPipeOrDuct && clusterSleeve.ClashZone != null && otherSleeve.ClashZone != null)
                            {
                                ...
                                if (cz1 != null && cz2 != null)
                                {
                                    ...
                                    if (logClusteringPath)
                                    {
                                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ========== CLUSTERING PATH SELECTION ==========\n");
                                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Checking: Sleeve {clusterSleeve.SleeveInstanceId} vs Sleeve {otherSleeve.SleeveInstanceId}\n");
                                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Sleeve {clusterSleeve.SleeveInstanceId} Rotation: {rotationAngle1 * 180.0 / Math.PI:F2}°\n");
                                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Sleeve {otherSleeve.SleeveInstanceId} Rotation: {rotationAngle2 * 180.0 / Math.PI:F2}°\n");
                                    }
                                    ...
                                    if (logClusteringPath)
                                    {
                                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Sleeve {clusterSleeve.SleeveInstanceId} IsRotated: {isRotated1}, IsAxisAligned: {IsAxisAlignedAngle(rotationAngle1)}\n");
                                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Sleeve {otherSleeve.SleeveInstanceId} IsRotated: {isRotated2}, IsAxisAligned: {IsAxisAlignedAngle(rotationAngle2)}\n");
                                    }
                                    ...
                                    if (isRotated1 && isRotated2)
                                    {
                                        ...
                                        if (logClusteringPath)
                                        {
                                            SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Both sleeves are rotated (non-axis-aligned)\n");
                                            SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Normalized angles: {angle1Deg:F2}° and {angle2Deg:F2}°\n");
                                            SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Axis difference: {angleDiffDeg:F2}° (0° or 180° = same axis)\n");
                                        }
                                        ...
                                        if (isSameAxis)
                                        {
                                            ...
                                            if (logClusteringPath)
                                            {
                                                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ✅ SELECTED: ROTATED CLUSTERING PATH (both sleeves on same axis: {rotationAngle1 * 180.0 / Math.PI:F2}°)\n");
                                            }
                                        }
                                        else
                                        {
                                            if (logClusteringPath)
                                            {
                                                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ⚠️ REJECTED: Rotated clustering (different axes: {angle1Deg:F2}° vs {angle2Deg:F2}°, diff={angleDiffDeg:F2}° - not 0° or 180°)\n");
                                                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Will use axis-aligned clustering instead\n");
                                            }
                                        }
                                    }
                                    else
                                    {
                                        if (logClusteringPath)
                                        {
                                            if (!isRotated1 && !isRotated2)
                                            {
                                                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Both sleeves are axis-aligned (rotation angles: {rotationAngle1 * 180.0 / Math.PI:F2}°, {rotationAngle2 * 180.0 / Math.PI:F2}°)\n");
                                            }
                                            else if (!isRotated1)
                                            {
                                                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Sleeve {clusterSleeve.SleeveInstanceId} is axis-aligned, Sleeve {otherSleeve.SleeveInstanceId} is rotated\n");
                                            }
                                            else
                                            {
                                                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Sleeve {clusterSleeve.SleeveInstanceId} is rotated, Sleeve {otherSleeve.SleeveInstanceId} is axis-aligned\n");
                                            }
                                        }
                                    }
                                }
                                else
                                {
                                    if (logClusteringPath)
                                    {
                                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ⚠️ ClashZone cast failed - cannot determine rotation\n");
                                    }
                                }
                            }
                            else
                            {
                                if (logClusteringPath)
                                {
                                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ⚠️ ClashZone is null for one or both sleeves - using axis-aligned clustering\n");
                                }
                            }
                            ...
                            if (shouldUseRotatedClustering)
                            {
                                ...
                                if (logClusteringPath)
                                {
                                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] 🔄 EXECUTING: ROTATED CLUSTERING PATH\n");
                                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Sleeve 1 axis: {rotationAngle1 * 180.0 / Math.PI:F2}°, Sleeve 2 axis: {rotationAngle2 * 180.0 / Math.PI:F2}°\n");
                                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Using Sleeve 1 axis direction: {axisRotationAngle * 180.0 / Math.PI:F2}° (NOT average)\n");
                                }
                                shouldCluster = CheckRotatedSleeveProximity(clusterSleeve, otherSleeve, axisRotationAngle, toleranceDist);
                                if (logClusteringPath)
                                {
                                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Rotated proximity check result: {shouldCluster}\n");
                                }
                            }
                            else
                            {
                                ...
                                if (logClusteringPath)
                                {
                                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] 📐 EXECUTING: AXIS-ALIGNED CLUSTERING PATH\n");
                                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Using BoundingBoxesOverlapFromXml\n");
                                }
                                ...
                                if (logClusteringPath)
                                {
                                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Axis-aligned proximity check result: {shouldCluster}\n");
                                }
                            }
                            
                            if (logClusteringPath)
                            {
                                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Final decision: {(shouldCluster ? "✅ CLUSTER" : "❌ NO CLUSTER")}\n");
                                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ========== END CLUSTERING PATH SELECTION ==========\n\n");
                            }
        ...
        /// </summary>
        [Obsolete("Use IClusterRotationService.DetermineRotationAngle")] // Phase 6 extraction
            ...
            string xmlFilePath = null)
        {
            try
            {
                ...
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ========== GET CLUSTER BOUNDING BOX WITH ROTATED COORDINATES ==========\n");
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Cluster contains {cluster.Count} sleeves\n");
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Rotation angle: {rotationAngle * 180.0 / Math.PI:F2}°\n");
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ========== READING INDIVIDUAL SLEEVE DATA ==========\n");
                
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Starting to process {cluster.Count} sleeves in cluster\n");
                int sleeveIndex = 0;
                foreach (var sleeveData in cluster)
                {
                    sleeveIndex++;
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Processing sleeve {sleeveIndex}/{cluster.Count}: SleeveInstanceId={sleeveData.SleeveInstanceId}\n");
                    
                    var clashZone = GetClashZoneBySleeveInstanceId(sleeveData.SleeveInstanceId, xmlFilePath);
                    if (clashZone != null)
                    {
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ClashZone found for sleeve {sleeveData.SleeveInstanceId}\n");
                        ...
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Sleeve {sleeveData.SleeveInstanceId}:\n");
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}]   Dimensions: W={sleeveWidth * 304.8:F1}mm, H={sleeveHeight * 304.8:F1}mm, D={sleeveDiameter * 304.8:F1}mm\n");
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}]   Placement Point: ({placementX:F6}, {placementY:F6}, {placementZ:F6})\n");
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}]   MEP Rotation Angle: {clashZone.MepElementRotationAngle * 180.0 / Math.PI:F2}°\n");
                        ...
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}]   Has Rotated BBox: {hasRotatedBbox}\n");
                        
                        // ✅ LOG: Axis-aligned bounding box
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}]   Axis-Aligned BBox: Min=({clashZone.SleeveBoundingBoxMinX:F6}, {clashZone.SleeveBoundingBoxMinY:F6}, {clashZone.SleeveBoundingBoxMinZ:F6}), Max=({clashZone.SleeveBoundingBoxMaxX:F6}, {clashZone.SleeveBoundingBoxMaxY:F6}, {clashZone.SleeveBoundingBoxMaxZ:F6})\n");
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}]   Axis-Aligned Size: W={(clashZone.SleeveBoundingBoxMaxX - clashZone.SleeveBoundingBoxMinX) * 304.8:F1}mm, H={(clashZone.SleeveBoundingBoxMaxY - clashZone.SleeveBoundingBoxMinY) * 304.8:F1}mm\n");
                        ...
                        if (hasRotatedBbox && cz != null)
                        {
                            ...
                            SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}]   Rotated BBox: Min=({cz.RotatedBoundingBoxMinX.Value:F6}, {cz.RotatedBoundingBoxMinY.Value:F6}, {cz.RotatedBoundingBoxMinZ ?? cz.SleeveBoundingBoxMinZ:F6}), Max=({cz.RotatedBoundingBoxMaxX.Value:F6}, {cz.RotatedBoundingBoxMaxY.Value:F6}, {cz.RotatedBoundingBoxMaxZ ?? cz.SleeveBoundingBoxMaxZ:F6})\n");
                            SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}]   Rotated Size: W={rotatedWidth:F1}mm, H={rotatedHeight:F1}mm\n");
                        ...
                        {
                            SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}]   ⚠️ No rotated bbox - using axis-aligned\n");
                            if (cz != null)
                            {
                        ...
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}]   ✅ Sleeve data loaded\n\n");
                    }
                    else
                    {
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ⚠️ ClashZone is NULL for SleeveInstanceId {sleeveData.SleeveInstanceId} - trying Revit API\n");
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}]   ⚠️ ClashZone not found for SleeveInstanceId {sleeveData.SleeveInstanceId} - trying Revit API\n");
                        ...
                        if (sleeve != null)
                        {
                            ...
                            if (bbox != null && bbox.Enabled)
                            {
                                ...
                                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}]   Revit BBox: Min=({bbox.Min.X:F6}, {bbox.Min.Y:F6}, {bbox.Min.Z:F6}), Max=({bbox.Max.X:F6}, {bbox.Max.Y:F6}, {bbox.Max.Z:F6})\n");
                                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}]   Revit Size: W={revitWidth:F1}mm, H={revitHeight:F1}mm\n");
                                axisAlignedBboxes.Add((bbox.Min, bbox.Max));
                            }
                        }
                        else
                        {
                            SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}]   ❌ Sleeve not found in Revit either!\n");
                        }
                    }
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Completed processing sleeve {sleeveIndex}/{cluster.Count}\n\n");
                }
                
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ========== FOREACH LOOP COMPLETED ==========\n");
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ========== SUMMARY: RotatedBboxes={rotatedBboxes.Count}, AxisAlignedBboxes={axisAlignedBboxes.Count}, HasRotatedBboxes={hasRotatedBboxes} ==========\n\n");
                
                // ✅ CRITICAL: Log all rotated bbox coordinates for debugging
                if (rotatedBboxes.Count > 0)
                {
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ========== ROTATED BBOX COORDINATES ==========\n");
                    for (int i = 0; i < rotatedBboxes.Count; i++)
                    {
                        ...
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] RotatedBbox[{i}]: Min=({bbox.min.X:F6}, {bbox.min.Y:F6}, {bbox.min.Z:F6}), Max=({bbox.max.X:F6}, {bbox.max.Y:F6}, {bbox.max.Z:F6}), Size=W={w:F1}mm, H={h:F1}mm\n");
                    }
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ========== END ROTATED BBOX COORDINATES ==========\n\n");
                }
                ...
                if (hasRotatedBboxes && rotatedBboxes.Count > 0)
                {
                    ...
                    {
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ========== PROPER CORNER-BASED ALGORITHM ==========\n");
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Processing {cluster.Count} sleeves - calculating all 4 corners in world space\n");
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Cluster INTENDED ROTATED AXIS (not average): {rotationAngle * 180.0 / Math.PI:F2}°\n");
                        ...
                        foreach (var sleeveData in cluster)
                        {
                            ...
                            {
                                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}]   ⚠️ WARNING: Sleeve {sleeveData.SleeveInstanceId} has zero rotation angle - check if MepElementRotationAngle was loaded from DB (column: MepRotationAngleRad)\n");
                            }
                            
                            // ✅ VERIFY: Log if pre-calculated corners are missing
                            if (preCalculatedCorners == null)
                            {
                                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}]   ⚠️ WARNING: Sleeve {sleeveData.SleeveInstanceId} has no pre-calculated corners - will recalculate (check if UpdateSleeveCorners was called during placement)\n");
                            }
                            
                            sleeveDataList.Add((cz, center, sleeveWidth, sleeveHeight, sleeveRotation, cosSleeve, sinSleeve, preCalculatedCorners));
                            
                            SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}]   Sleeve {sleeveData.SleeveInstanceId}: Center=({center.X:F6}, {center.Y:F6}, {center.Z:F6}), W={sleeveWidth*304.8:F1}mm, H={sleeveHeight*304.8:F1}mm, Rotation={sleeveRotation*180.0/Math.PI:F2}°, HasPreCalcCorners={preCalculatedCorners != null}, HasPreCalcCosSin={cosSleeve.HasValue && sinSleeve.HasValue}\n");
                        }
                        ...
                        if (sleeveDataList.Count == 0)
                        {
                            ...
                            width = maxX - minX;
                            height = maxY - minY;
                            SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ⚠️ No sleeve data found - using fallback union\n");
                        }
                        else
                        {
                            ...
                            // ✅ PHASE 1: Use RotationMatrixCalculator with crash-safe guards
                            var rotationMatrix = RotationMatrixCalculator.CreateRotationMatrix(rotationAngle);
                            if (rotationMatrix == null)
                            {
                                SafeFileLogger.SafeAppendText("cluster_debug.log",
                                    $"[{DateTime.Now:HH:mm:ss.fff}] ⚠️ Failed to create rotation matrix for angle {rotationAngle * 180.0 / Math.PI:F2}° - falling back to axis-aligned\n");
                                rotationAngle = 0.0; // Fallback to axis-aligned
                                rotationMatrix = RotationMatrixCalculator.CreateRotationMatrix(0.0);
                            }
                            double cosCluster = rotationMatrix.Value.cos;
                            double sinCluster = rotationMatrix.Value.sin;
                            
                            SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Origin (first sleeve center): ({origin.X:F6}, {origin.Y:F6}, {origin.Z:F6})\n");
                            SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Cluster INTENDED AXIS Rotation Matrix: cos={cosCluster:F6}, sin={sinCluster:F6}\n");
                            ...
                            for (int i = 0; i < sleeveDataList.Count; i++)
                            {
                                ...
                                if (preCalculatedCorners != null)
                                {
                                    ...
                                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}]   Sleeve {i+1} Using PRE-CALCULATED corners from database\n");
                                }
                                else
                                {
                                    // ✅ FALLBACK: Recalculate corners if pre-calculated ones are missing
                                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}]   Sleeve {i+1} Recalculating corners (pre-calculated corners not available)\n");
                                ...
                                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}]   Sleeve {i+1} World Corners: ({worldCorners[0].X:F6}, {worldCorners[0].Y:F6}), ({worldCorners[1].X:F6}, {worldCorners[1].Y:F6}), ({worldCorners[2].X:F6}, {worldCorners[2].Y:F6}), ({worldCorners[3].X:F6}, {worldCorners[3].Y:F6})\n");
                            ...
                            SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ✅ CORRECT UNION (from all {allTransformedCorners.Count} corner points): MinX={minX:F6}, MinY={minY:F6}, MaxX={maxX:F6}, MaxY={maxY:F6}\n");
                            SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ✅ CLUSTER SIZE (WATERTIGHT ALGORITHM): W={width * 304.8:F1}mm, H={height * 304.8:F1}mm\n");
                        }
                        
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Cluster BBox: Min=({minX:F6}, {minY:F6}), Max=({maxX:F6}, {maxY:F6})\n\n");
                    ...
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ========== FINAL CLUSTER DIMENSIONS (BEFORE TRANSFORM) ==========\n");
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Width: {width:F6} = {widthMm:F1}mm\n");
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Height: {height:F6} = {heightMm:F1}mm\n");
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Depth: {depth:F6} = {depthMm:F1}mm\n");
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] BBox Min: ({minX:F6}, {minY:F6}, {minZ:F6})\n");
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] BBox Max: ({maxX:F6}, {maxY:F6}, {maxZ:F6})\n\n");
                    ...
                    try
                    {
                        SafeFileLogger.SafeAppendText("cluster_debug.log", logBuilder.ToString());
                    }
                    catch { }
                    ...
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ========== MIDPOINT CALCULATION ==========\n");
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Midpoint (in rotated coordinate space): ({midRotated.X:F6}, {midRotated.Y:F6}, {midRotated.Z:F6})\n");
                    ...
                    if (allBboxes.Count > 0 && Math.Abs(rotationAngle) > 1e-6 && origin != XYZ.Zero)
                    {
                        ...
                        {
                            SafeFileLogger.SafeAppendText("cluster_debug.log",
                                $"[{DateTime.Now:HH:mm:ss.fff}] ⚠️ Failed to create inverse rotation matrix - using axis-aligned midpoint\n");
                            mid = new XYZ(
                        ...
                        {
                            double cosA = inverseRotationMatrix.Value.cos;  // Use same angle (inverse rotation matrix is transpose)
                            double sinA = inverseRotationMatrix.Value.sin;
                            // ✅ INVERSE TRANSFORMATION: Use CoordinateTransformer for inverse rotation
                            var inversePoint = RotationMatrixCalculator.InverseRotation((midRotated.X, midRotated.Y), cosA, sinA);
                            if (inversePoint == null)
                            {
                                SafeFileLogger.SafeAppendText("cluster_debug.log",
                                    $"[{DateTime.Now:HH:mm:ss.fff}] ⚠️ Failed to apply inverse rotation - using axis-aligned midpoint\n");
                                mid = new XYZ(
                                    origin.X + midRotated.X,
                                    origin.Y + midRotated.Y,
                                    midRotated.Z
                        ...
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ========== TRANSFORM CALCULATION ==========\n");
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Rotation Angle: {rotationAngle * 180.0 / Math.PI:F2}°\n");
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Origin: ({origin.X:F6}, {origin.Y:F6}, {origin.Z:F6})\n");
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Midpoint (after transform to world): ({mid.X:F6}, {mid.Y:F6}, {mid.Z:F6})\n\n");
                    }
                    else
                    {
                        ...
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] No rotation applied (rotationAngle={rotationAngle * 180.0 / Math.PI:F2}°)\n");
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Midpoint (world coordinates): ({mid.X:F6}, {mid.Y:F6}, {mid.Z:F6})\n\n");
                    }
                    
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ========== RETURNING CLUSTER BOUNDING BOX ==========\n");
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Final: W={widthMm:F1}mm, H={heightMm:F1}mm, D={depthMm:F1}mm, Mid=({mid.X:F6}, {mid.Y:F6}, {mid.Z:F6})\n");
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Rotated BBox: Min=({minX:F6}, {minY:F6}, {minZ:F6}), Max=({maxX:F6}, {maxY:F6}, {maxZ:F6})\n");
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Internal units: W={width:F6}, H={height:F6}, D={depth:F6}\n");
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ========== END GET CLUSTER BOUNDING BOX ==========\n\n");
            ...
            {
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ❌❌❌ EXCEPTION IN GetClusterBoundingBoxWithRotatedCoordinates ❌❌❌\n");
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Exception Type: {ex.GetType().Name}\n");
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Error Message: {ex.Message}\n");
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] StackTrace: {ex.StackTrace}\n");
                if (ex.InnerException != null)
                {
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Inner Exception: {ex.InnerException.GetType().Name} - {ex.InnerException.Message}\n");
                }
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ⚠️ Falling back to ClusterBoundingBoxServices.GetClusterBoundingBox\n");
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ========== END EXCEPTION ==========\n\n");
            ...
            try
            {
                ...
                foreach (var sleeve in cluster)
                {
                    ...
                    if (bbox != null)
                    {
                        ...
                        if (!DeploymentConfiguration.DeploymentMode)
                        {
                            SafeFileLogger.SafeAppendText("cluster_debug.log", $"  Sleeve {sleeve.SleeveInstanceId}: W={sleeveWidth:F1}mm, H={sleeveHeight:F1}mm, D={sleeveDepth:F1}mm, " +
                    ...
                    for (int j = 0; j < i; j++)
                    {
                        ...
                        if (!processedPairs.Contains(pairKey))
                        {
                            ...
                            if (overlapVol > 0)
                            {
                                ...
                                if (!DeploymentConfiguration.DeploymentMode)
                                {
                                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[OVERLAP] Sleeves {i} and {j}: Overlap Vol={UnitUtils.ConvertFromInternalUnits(overlapVol, UnitTypeId.CubicMillimeters):F2}mm³, " +
        ...
        private bool BoundingBoxesOverlapFromXml(dynamic sleeve1, dynamic sleeve2, double toleranceDist)
        {
            try
            {
                ...
                if (isRoundPipeOrDuct)
                {
                    ...
                    if (placement1 != null && placement2 != null && radius1 > 0 && radius2 > 0)
                    {
                        ...
                        if (!DeploymentConfiguration.DeploymentMode)
                        {
                            ...
                            try 
                            { 
                                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss}] [ROUND-PIPE-PROXIMITY] Sleeve {sleeve1.SleeveInstanceId} vs {sleeve2.SleeveInstanceId}: " +
                ...
                { 
                    ...
                    SafeFileLogger.SafeAppendText("placement_debug.log", $"[{DateTime.Now:HH:mm:ss}] [CLUSTERING-DISTANCE] Sleeve {sleeve1.SleeveInstanceId} vs {sleeve2.SleeveInstanceId}: " +
        ...
        private List<List<FamilyInstance>> CalculateClustersUsingBoundingBoxOverlap(List<FamilyInstance> sleeves, double toleranceDist, string orientation)
        {
            ...
            while (unprocessedSet.Count > 0)
            {
                ...
                if (cluster.Count > 1)
                {
                    ...
                    if (!DeploymentConfiguration.DeploymentMode)
                    {
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"✓ CLUSTER: {string.Join(", ", cluster.Select(s => s.Id.IntegerValue))}\n");
                    }
                }
                else
                {
        ...
        private bool CheckRotatedSleeveProximity(dynamic sleeve1, dynamic sleeve2, double rotationAngle, double toleranceDist)
        {
            try
            {
                ...
                if (shouldLog)
                {
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ========== CHECK ROTATED SLEEVE PROXIMITY ==========\n");
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Sleeve {sleeve1.SleeveInstanceId} vs Sleeve {sleeve2.SleeveInstanceId}\n");
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Rotation Angle: {rotationAngle * 180.0 / Math.PI:F2}°\n");
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Tolerance: {toleranceDist * 304.8:F1}mm\n");
                }
                ...
                if (sleeve1?.ClashZone == null || sleeve2?.ClashZone == null)
                {
                    if (shouldLog)
                    {
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ❌ ClashZone is null - returning false\n\n");
                    }
                    return false;
                }
                ...
                if (cz1 == null || cz2 == null)
                {
                    if (shouldLog)
                    {
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ❌ ClashZone cast failed - returning false\n\n");
                    }
                    return false;
                }
                ...
                if (shouldLog)
                {
                    ...
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Sleeve {sleeve1.SleeveInstanceId} Center: ({center1.X:F6}, {center1.Y:F6}, {center1.Z:F6}) [{(hasPlacementPoint1 ? "from PlacementPoint" : "from BBox center")}]\n");
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Sleeve {sleeve2.SleeveInstanceId} Center: ({center2.X:F6}, {center2.Y:F6}, {center2.Z:F6}) [{(hasPlacementPoint2 ? "from PlacementPoint" : "from BBox center")}]\n");
                }
                ...
                if (cz1.MepElementOrientation != null && cz1.MepElementOrientation.GetLength() > 1e-6)
                {
                    ...
                    if (shouldLog)
                    {
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Rotated Axis Direction (from MepElementOrientation): ({rotatedAxisDirection.X:F6}, {rotatedAxisDirection.Y:F6}, {rotatedAxisDirection.Z:F6})\n");
                    }
                }
                else
                {
                    ...
                    if (shouldLog)
                    {
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Rotated Axis Direction (fallback from angle): ({rotatedAxisDirection.X:F6}, {rotatedAxisDirection.Y:F6}, {rotatedAxisDirection.Z:F6})\n");
                    }
                }
                ...
                if (shouldLog)
                {
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Rotated Axis Direction: ({rotatedAxisDirection.X:F6}, {rotatedAxisDirection.Y:F6}, {rotatedAxisDirection.Z:F6})\n");
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] World Vector (center1 to center2): ({worldVector.X:F6}, {worldVector.Y:F6}, {worldVector.Z:F6})\n");
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Sleeve 1 Size: W={sleeve1Width * 304.8:F1}mm, H={sleeve1Height * 304.8:F1}mm\n");
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Sleeve 2 Size: W={sleeve2Width * 304.8:F1}mm, H={sleeve2Height * 304.8:F1}mm\n");
                }
                ...
                if (shouldLog)
                {
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Distance Along Axis: {distanceAlongAxis * 304.8:F1}mm (max allowed: {maxDistanceAlongAxis * 304.8:F1}mm) - {(closeAlongAxis ? "✅ PASS" : "❌ FAIL")}\n");
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Perpendicular Distance: {perpendicularDistance * 304.8:F1}mm (max allowed: {maxPerpendicularDistance * 304.8:F1}mm) - {(closePerpendicular ? "✅ PASS" : "❌ FAIL")}\n");
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] Final Result: {(shouldCluster ? "✅ CLUSTER" : "❌ NO CLUSTER")}\n");
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss.fff}] ========== END CHECK ROTATED SLEEVE PROXIMITY ==========\n\n");
                }
        ...
        /// </summary>
        /// <summary>
        /// ✅ PHASE 1 REFACTORING: Wrapper for DistanceCalculator.CalculateMinimumDistance2D
        /// Maintains backward compatibility while using new crash-safe geometry classes.
        /// </summary>
        [Obsolete("Use DistanceCalculator.CalculateMinimumDistance2D directly. This wrapper maintains backward compatibility during migration.")]
        ...
        {
            // ✅ PHASE 1: Use new DistanceCalculator with crash-safe guards
            double? distance = DistanceCalculator.CalculateMinimumDistance2D(
                minX1, minY1, maxX1, maxY1,
                minX2, minY2, maxX2, maxY2);
            
            // ✅ CRASH-SAFE: Fallback to 0 if calculation failed (null return)
            return distance ?? 0.0;
        }

        /// <summary>
        /// ✅ PHASE 1 REFACTORING: Wrapper for DistanceCalculator.CalculateMinimumDistance3D
        /// Maintains backward compatibility while using new crash-safe geometry classes.
        /// </summary>
        [Obsolete("Use DistanceCalculator.CalculateMinimumDistance3D directly. This wrapper maintains backward compatibility during migration.")]
        ...
        {
            // ✅ PHASE 1: Use new DistanceCalculator with crash-safe guards
            double? distance = DistanceCalculator.CalculateMinimumDistance3D(
            ...
            SleeveGroupKey groupKey)
        {
            ...
            while (unprocessedSet.Count > 0)
            {
                ...
                if (cluster.Count > 1)
                {
                    ...
                    if (!DeploymentConfiguration.DeploymentMode)
                    {
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"✅ CLUSTER FORMED: {cluster.Count} sleeves [{clusterIds}]\n");
                    }
                }
                else
                {
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"❌ INDIVIDUAL SLEEVE: {cluster[0].Id.IntegerValue} (no proximate neighbors)\n");
                }
            ...
            if (!DeploymentConfiguration.DeploymentMode)
            {
            SafeFileLogger.SafeAppendText("cluster_debug.log", $"Total clusters formed: {totalClusters}\n");
            }
                        // ✅ DEPLOYMENT MODE: Skip file writes
            if (!DeploymentConfiguration.DeploymentMode)
            {
            SafeFileLogger.SafeAppendText("cluster_debug.log", $"Total individual sleeves: {totalIndividuals}\n");
            }
            SafeFileLogger.SafeAppendText("cluster_debug.log", $"Total sleeves processed: {groupClusters.Sum(c => c.Count)}\n\n");
        ...
        /// </summary>
        [Obsolete("Use IClusterDataService.LoadClashZonesFromRegularXml instead")]
                ...
                if (!string.IsNullOrEmpty(targetCategory) && doc != null)
                {
                        ...
                        using (var dbContext = new SleeveDbContext(doc))
                        {
                            ...
                            if (dbZones != null && dbZones.Count > 0)
                            {
                                ...
                                if (!DeploymentConfiguration.DeploymentMode)
                                {
                                    ...
                                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss}] [DB-LOAD] Category={targetCategory}, Loaded={clashZones.Count}, TotalInDB={dbZones.Count}\n");
                                }
                            ...
                            else
                            {
                                if (!DeploymentConfiguration.DeploymentMode)
                                {
                                    ...
                                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss}] [DB-LOAD] Category={targetCategory}, Loaded=0 (falling back to XML)\n");
                                }
                            }
                        }
                    }
                    catch (Exception dbEx)
                    {
                        if (!DeploymentConfiguration.DeploymentMode)
                        {
                            ...
                            SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss}] [DB-LOAD] Category={targetCategory}, Error={dbEx.Message} (falling back to XML)\n");
                        }
                    }
                }
                        ...
                        if (File.Exists(xmlFile))
                        {
                            ...
                            using (var reader = new StreamReader(xmlFile))
                            {
                                ...
                                if (!DeploymentConfiguration.DeploymentMode)
                                {
                                    ...
                                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[{DateTime.Now:HH:mm:ss}] [XML-LOAD] File={Path.GetFileName(xmlFile)}, Loaded={hierarchicalCount + flatCount}, Skipped={skippedNoSleeveId} (no SleeveId) + {skippedCategoryMismatch} (category), TotalInFile={totalZonesInFile}\n");
                                }
                            }
                        }
                        else if (!DeploymentConfiguration.DeploymentMode)
        ...
        /// </summary>
        [Obsolete("Use IClusterDataService.LoadClashZoneCacheForCleanup instead")]
        public void LoadClashZoneCacheForCleanup(string xmlFilePath, string targetCategory, Document doc, string filterName)
        {
            ...
            if (!DeploymentConfiguration.DeploymentMode)
            {
                ...
                // Log to file as well
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[CACHE-RELOAD] Total: {_clashZoneCache.Count}, WithClusterId: {withClusterId}, WithClusterBbox: {withClusterBbox}\n");
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[CACHE-RELOAD] By host type: {string.Join(", ", byHostType.Select(kvp => $"{kvp.Key}={kvp.Value}"))}\n");
            }
        }
        
        /// <summary>
        /// Public method to run cleanup after XML save.
        /// Routes through Phase 7 service if available, otherwise legacy method.
        /// </summary>
        [Obsolete("Use IClusterCleanupService.CleanupSleevesWithinClusters directly and call SaveUpdatedFlagsToDb separately")] // Phase 7
        public int CleanupSleevesWithinClustersAfterDbSave(Document doc, List<FamilyInstance> placedClusters, string xmlFilePath = null)
        {
            int deleted = _cleanupService != null
                ? _cleanupService.CleanupSleevesWithinClusters(doc, placedClusters)
                : CleanupSleevesWithinClusters(doc, placedClusters); // fallback legacy
        ...
        /// </summary>
        [Obsolete("Use IClusterDataService.LoadClashZoneCacheFromLoadedClashZones instead")]
            ...
            string xmlFilePath = null)
        {
            ...
            if (!DeploymentConfiguration.DeploymentMode)
            {
                ...
                SafeFileLogger.SafeAppendText("cluster_debug.log", groupKeyMsg);
            }
            ...
            else
            {
                if (!DeploymentConfiguration.DeploymentMode)
                {
                    ...
                    SafeFileLogger.SafeAppendText("cluster_debug.log", earlyReturnMsg);
                }
            ...
            if (!DeploymentConfiguration.DeploymentMode)
            {
                ...
                SafeFileLogger.SafeAppendText("cluster_debug.log", familyMsg);
            }
            ...
            if (groupKey.systemType != null)
            {
                ...
                if (isPipeForRotation)
                {
                    ...
                    if (!DeploymentConfiguration.DeploymentMode)
                    {
                        ...
                        SafeFileLogger.SafeAppendText("cluster_debug.log", skipMsg);
                    }
                }
            ...
            try
            {
                ...
                if (!DeploymentConfiguration.DeploymentMode)
                {
                    ...
                    SafeFileLogger.SafeAppendText("cluster_debug.log", placementMsg);
                }
            }
            ...
            try
            {
                SafeFileLogger.SafeAppendText("cluster_debug.log", sizeLogBuilder.ToString());
            }
            catch { }
            ...
            if (!DeploymentConfiguration.DeploymentMode)
            {
                ...
                SafeFileLogger.SafeAppendText("cluster_debug.log", setMsg);
            }
            ...
            if (!DeploymentConfiguration.DeploymentMode)
            {
                ...
                SafeFileLogger.SafeAppendText("cluster_debug.log", verifyMsg);
            }
                ...
                try
                {
                    ...
                    if (sleeveElement != null && sleeveElement is FamilyInstance sleeveInstance)
                    {
                        ...
                        if (!DeploymentConfiguration.DeploymentMode)
                        {
                            ...
                            SafeFileLogger.SafeAppendText("cluster_debug.log", $"[DELETE-CLUSTER-FORMATION] ✅ Queued cluster formation sleeve {sleeveElementId.IntegerValue} for deletion\n");
                        }
                    }
                    else
                    {
            ...
            if (!DeploymentConfiguration.DeploymentMode)
            {
                ...
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[DELETE-CLUSTER-FORMATION] Total cluster formation sleeves: {sleevesToDelete.Count} (IDs: {string.Join(", ", clusterFormationSleeveIds)})\n");
            }
            ...
            if (clusterBbox != null && !string.IsNullOrEmpty(targetCategory))
            {
                try
                {
                    ...
                    if (!DeploymentConfiguration.DeploymentMode)
                    {
                        ...
                        SafeFileLogger.SafeAppendText("cluster_debug.log", deleteSummary);
                ...
                catch (Exception edgeEx)
                {
                    if (!DeploymentConfiguration.DeploymentMode)
                    {
                        ...
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[DELETE-EDGE-CASE-ERROR] ❌ Error deleting edge case sleeves: {edgeEx.Message}\n");
                    }
                }
            }
        }
            ...
            bool shouldSwapDimensions = false)
        {
            ...
            if (groupKey.hostType == "Wall" || groupKey.hostType == "Structural Framing")
            {
                ...
                if (!DeploymentConfiguration.DeploymentMode)
                {
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[CLUSTER-DIM] Sleeve {inst.Id}: Orientation={groupKey.orientation}, Swap={shouldSwapDimensions}, " +
            ...
            try
            {
                SafeFileLogger.SafeAppendText("cluster_debug.log", swapLogBuilder.ToString());
            }
            catch { }
        }
        ...
        private void SetClusterSleeveMetadata(FamilyInstance clusterSleeve, string category)
    {
        try
        {
            ...
            if (!DeploymentConfiguration.DeploymentMode)
            {
                ...
                SafeFileLogger.SafeAppendText("cluster_debug.log", filterNameLog);
            }
            ...
            {
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[SetClusterSleeveMetadata] ✅ GetFilterNameForCategory returned: '{filterName}' for category '{category}'\n");
            }
    ...
    /// </summary>
    [Obsolete("Use IClusterCleanupService.CleanupSleevesWithinClusters")] // Phase 7
    private int CleanupSleevesWithinClusters(Document doc, List<FamilyInstance> placedClusters)
    {
        ...
        try
        {
            ...
            if (!DeploymentConfiguration.DeploymentMode)
            {
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[CLEANUP-START] Cluster sleeve IDs: {string.Join(", ", placedClusters.Select(c => c.Id.IntegerValue))}\n");
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[CLEANUP-PROTECTION-SET] Protection set contains {clusterSleeveIds.Count} cluster sleeve IDs: {string.Join(", ", clusterSleeveIds)}\n");
            ...
            if (!DeploymentConfiguration.DeploymentMode)
            {
                SafeFileLogger.SafeAppendText("cluster_debug.log", $"[CLEANUP-ALL-SLEEVES-FOUND] Found {allSleeves.Count} total sleeves in Revit: {string.Join(", ", allSleeves.Select(s => s.Id.IntegerValue))}\n");
            }
            ...
            var individualSleeves = allSleeves.Where(s => 
            {
                ...
                if (clusterSleeveIds.Contains(sleeveId))
                {
                    ...
                    if (!DeploymentConfiguration.DeploymentMode)
                    {
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[CLEANUP-PARAM-CHECK] ⚠️ Sleeve {sleeveId} is in protection set - FORCING isClusterSleeve=true (SleeveInstanceId={sleeveInstanceId}, ClusterValue={clusterValue})\n");
                    }
                }
            ...
            foreach (var individualSleeve in individualSleeves)
            {
                ...
                if (clusterSleeveIds.Contains(sleeveId))
                {
                                        if (!DeploymentConfiguration.DeploymentMode)
                    {
                        ...
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[CLEANUP-TERTIARY] ⚠️ PROTECTED: Sleeve {sleeveId} is in protection set - SKIPPING deletion\n");
                    }
            ...
            if (sleevesToDelete.Count > 0)
            {
                ...
                if (!DeploymentConfiguration.DeploymentMode)
                {
                    SafeFileLogger.SafeAppendText("cluster_debug.log", $"[CLEANUP-BATCH-DELETE] Marked {sleevesToDelete.Count} sleeves for batch deletion: {string.Join(", ", sleevesToDelete.Select(s => s.sleeveId.IntegerValue))}\n");
                    if (clusterSleevesInDeletionList.Count > 0)
                    {
                        SafeFileLogger.SafeAppendText("cluster_debug.log", $"[CLEANUP-BATCH-DELETE] ⚠️ Removed {clusterSleevesInDeletionList.Count} cluster sleeves from deletion list\n");
                    }
                }
