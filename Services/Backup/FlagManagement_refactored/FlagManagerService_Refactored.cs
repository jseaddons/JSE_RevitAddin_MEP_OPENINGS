    ...
    public class FlagManagerService : IFlagManager
    {
            ...
            string filterName = null)
        {
            ...
            try
            {
                ...
                // ✅ TESTABILITY: Use injected IClashZoneRepository (can be mocked)
                _logger.Info($"📝 BATCH: Calling BatchUpdateFlags for {dbUpdates.Count} clash zones, IsCluster={isCluster}", "FlagManager");
                
                _repository.BatchUpdateFlags(dbUpdates);
                
                _logger.Info($"✅ BATCH: Updated database flags for {dbUpdates.Count} clash zones", "FlagManager");
            }
            catch (Exception ex)
            {
