...
namespace JSE_RevitAddin_MEP_OPENINGS.Services
{
    ...
    public class CableTraySleevePlacer
    {
            ...
            Wall hostWall)
        {
            ...
            // FITTING SUPPRESSION: skip if a fitting is found at the intersection
            if (IsFittingAtIntersection(_doc, intersection, tray, cableTrayId))
            {
                DebugLogger.Log($"[CableTraySleevePlacer] Fitting detected at intersection for cable tray {cableTrayId}, skipping sleeve placement.");
                SleeveLogManager.LogCableTraySleeveFailure(cableTrayId, "Fitting present at intersection");
                return;
            }
        ...
        private XYZ GetWallCenterlinePoint(Wall wall, XYZ intersectionPoint)
        {
            ...
            try
            {
                ...
                if (locationCurve?.Curve is Line wallCenterline)
                {
                    ...
                    Autodesk.Revit.DB.IntersectionResult projectionResult = wallCenterline.Project(intersectionPoint);
        ...
        private bool IsFittingAtIntersection(Document doc, XYZ intersection, CableTray tray, int cableTrayId)
        {
            ...
            foreach (FamilyInstance fi in collector)
            {
                ...
                if (!isFitting)
                {
                    DebugLogger.Log($"[CableTraySleevePlacer][FittingCheck] CableTray {cableTrayId}: Skipping non-fitting {fi.Id.IntegerValue}, family={famName}, type={typeName}");
                    continue;
                }
                ...
                {
                    DebugLogger.Log($"[CableTraySleevePlacer][FittingCheck] CableTray {cableTrayId}: Fitting {fi.Id.IntegerValue} has unsupported location type: {fi.Location?.GetType().Name ?? "null"}");
                    continue;
                }

                DebugLogger.Log($"[CableTraySleevePlacer][FittingCheck] CableTray {cableTrayId}: Fitting {fi.Id.IntegerValue} at dist={dist:F3}, family={famName}, type={typeName}, locationType={locationType}");
                if (dist < searchRadius)
                {
                    DebugLogger.Log($"[CableTraySleevePlacer][FittingCheck] CableTray {cableTrayId}: SUPPRESSION TRIGGERED by fitting {fi.Id.IntegerValue} at dist={dist:F3}");
                    foundSuppression = true;
                }
            }
            if (!foundSuppression)
            {
                DebugLogger.Log($"[CableTraySleevePlacer][FittingCheck] CableTray {cableTrayId}: No fitting found within suppression radius at intersection {intersection}");
            }
