...
namespace JSE_RevitAddin_MEP_OPENINGS.Services
{
    ...
    public class NewSleevePlacerService
    {
        ...
        private (int placed, int skipped, int errors) ExecutePlacementInternal(List<ClashZone> clashZones)
        {
            ...
            foreach (var clashZone in filteredZones)
            {
                ...
                try
                {
                    ...
                    // ? PLACEMENT STEP 3: Place the family instance
                    // This is the actual Revit transaction wrapping creation
        ...
        private FamilyInstance PlaceSleeveFromSavedData(ClashZone zone)
        {
            ...
            if (instance != null)
            {
                ...
                if (zone.ClusterSleeveInstanceId <= 0)
                {
                    ...
                    // ? CACHE: Update calculated placement point in memory so it's persisted to DB
        ...
        private FamilyInstance PlaceSleeveNormal(ClashZone zone, SleevePlacementPlanningDto? planningDto = null)
        {
            ...
            bool hasSavedPoint = (zone.SleevePlacementPointX != 0 || zone.SleevePlacementPointY != 0 || zone.SleevePlacementPointZ != 0);
            ...
            double rotation = (zone.MepElementRotationAngle != 0)
                ? zone.MepElementRotationAngle
                : _rotationService.DetermineRotation(zone);

            // ? CRITICAL: Persist the CALCULATED placement point for future optimization
            // This allows skipping the expensive AdjustPlacementPoint calculation on subsequent runs
            zone.CalculatedPlacementX = placementPoint.X;
            zone.CalculatedPlacementY = placementPoint.Y;
            zone.CalculatedPlacementZ = placementPoint.Z;
        ...
        private (double width, double height, double diameter, bool isCircular) CalculateSleeveDimensions(ClashZone zone, SleevePlacementPlanningDto? planningDto = null)
        {
            ...
            {
                // ✅ PRIORITY 0: Use DB-cached dimensions (FASTEST - ~0.5ms)
                // These are populated during calculation phase and stored in ClashZones table (CalculatedSleeve... columns)
                // Eliminates clearance lookup (~5-8ms) + rounding calculation (~3-5ms) + strategy logic (~8-12ms)
                bool hasCachedDimensions = (zone.CalculatedSleeveWidth > 0.0 || zone.CalculatedSleeveHeight > 0.0 || zone.CalculatedSleeveDiameter > 0.0);
                
                if (hasCachedDimensions && OptimizationFlags.UseDimensionCache)
                {
                    if (!DeploymentConfiguration.DeploymentMode && !OptimizationFlags.DisableVerboseLogging)
                    {
                        SafeFileLogger.SafeAppendText("placement_debug.log",
                            $"[FAST PATH] Using pre-calculated dimensions from DB: " +
                            $"W={zone.CalculatedSleeveWidth * 304.8:F1}mm, H={zone.CalculatedSleeveHeight * 304.8:F1}mm, " +
                            $"D={zone.CalculatedSleeveDiameter * 304.8:F1}mm\n");
                    }
                    
                    tracker?.SetItemCount(1);
                    bool cachedIsCircular = zone.CalculatedSleeveDiameter > 0.0;
                    return (zone.CalculatedSleeveWidth, zone.CalculatedSleeveHeight, zone.CalculatedSleeveDiameter, cachedIsCircular);
                }
                ...
                // ? PERFORMANCE MONITORING: Track dimension calculation
                using (_performanceMonitor?.TrackOperation("Calculate Sleeve Dimensions"))
                {
                    (finalWidth, finalHeight, finalDiameter) = _sizingService.CalculateFinalDimensionsFromClashZoneRounded(
