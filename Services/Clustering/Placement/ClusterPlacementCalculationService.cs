...
namespace JSE_RevitAddin_MEP_OPENINGS.Services.Clustering.Placement
{
    ...
    public class ClusterPlacementCalculationService : IClusterPlacementCalculationService
    {
            ...
            string? xmlFilePath = null)
        {
            ...
            return placementPoint ?? XYZ.Zero;
        }
        ...
        private XYZ ApplyDamperAlignment(XYZ currentPoint, List<dynamic> cluster, string? xmlFilePath)
        {
            ...
            // Get First Valid ClashZone for Wall Information
            ClashZone? firstValidCz = null;
            foreach (var sleeve in cluster)
            {
                int id = GetSleeveId(sleeve);
                var cz = _getClashZoneFunc(id, xmlFilePath);
                if (cz != null) { firstValidCz = cz; break; }
            }

            if (firstValidCz != null && IsWallHosted(firstValidCz))
            {
                // ✅ LATERAL SHIFT FIX: Use WallCenterline for alignment axis
                // Preserve Dimension Axis (Length) from currentPoint (which might be Geometric Center)
                ...
                {
                    // Y-Wall: Thickness is X. Length is Y.
                    // Lock X to Wall Centerline. Preserve Y (Geometric Center).
                    cX = firstValidCz.WallCenterlinePointX;
                    if (!DeploymentConfiguration.DeploymentMode)
                        SafeFileLogger.SafeAppendText("cluster_sizing.log", $"[{DateTime.Now:HH:mm:ss}] 🎯 PLACEMENT (DAMPER ALIGN): Y-Wall Fixed X={cX:F4} (Core), Kept Y={cY:F4} (Geometric)\n");
                }
                else if (orientation.Contains("X")) // Normal along Y, Thickness along Y
                {
                    // X-Wall: Thickness is Y. Length is X.
                    // Lock Y to Wall Centerline. Preserve X (Geometric Center).
                    cY = firstValidCz.WallCenterlinePointY;
                    if (!DeploymentConfiguration.DeploymentMode)
                        SafeFileLogger.SafeAppendText("cluster_sizing.log", $"[{DateTime.Now:HH:mm:ss}] 🎯 PLACEMENT (DAMPER ALIGN): X-Wall Fixed Y={cY:F4} (Core), Kept X={cX:F4} (Geometric)\n");
                }
        ...
        private XYZ CalculateFromIntersections(List<dynamic> cluster, string? xmlFilePath)
        {
            ...
            if (validCount == 0 || firstValidCz == null) return XYZ.Zero;
