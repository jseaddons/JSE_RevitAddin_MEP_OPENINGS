...
namespace JSE_RevitAddin_MEP_OPENINGS.Services
{
	...
	public class CategoryGlobalIndexEntry
	{
		...
		[XmlAttribute("SleeveWidth")] public double SleeveWidth { get; set; }
		[XmlAttribute("SleeveHeight")] public double SleeveHeight { get; set; }
		[XmlAttribute("SleeveDiameter")] public double SleeveDiameter { get; set; }
		[XmlAttribute("SleevePlacementPointX")] public double SleevePlacementPointX { get; set; }
		[XmlAttribute("SleevePlacementPointY")] public double SleevePlacementPointY { get; set; }
		[XmlAttribute("SleevePlacementPointZ")] public double SleevePlacementPointZ { get; set; }
		[XmlAttribute("SleevePlacementPointActiveDocumentX")] public double SleevePlacementPointActiveDocumentX { get; set; }
		[XmlAttribute("SleevePlacementPointActiveDocumentY")] public double SleevePlacementPointActiveDocumentY { get; set; }
		[XmlAttribute("SleevePlacementPointActiveDocumentZ")] public double SleevePlacementPointActiveDocumentZ { get; set; }
		[XmlAttribute("SleeveBoundingBoxMinX")] public double SleeveBoundingBoxMinX { get; set; }
		[XmlAttribute("SleeveBoundingBoxMinY")] public double SleeveBoundingBoxMinY { get; set; }
		[XmlAttribute("SleeveBoundingBoxMinZ")] public double SleeveBoundingBoxMinZ { get; set; }
		[XmlAttribute("SleeveBoundingBoxMaxX")] public double SleeveBoundingBoxMaxX { get; set; }
		[XmlAttribute("SleeveBoundingBoxMaxY")] public double SleeveBoundingBoxMaxY { get; set; }
		[XmlAttribute("SleeveBoundingBoxMaxZ")] public double SleeveBoundingBoxMaxZ { get; set; }
	...
	public static void UpsertFlagsWithIdsAndClashZoneData(Document doc, string categoryName, IEnumerable<(Guid Id, bool IsResolved, bool IsClusterResolved, int SleeveInstanceId, int ClusterSleeveInstanceId, int MepElementId, int StructuralElementId, double IntersectionPointX, double IntersectionPointY, double IntersectionPointZ)> updates, string filterName = null)
		...
		var map = allEntries.ToDictionary(e => e.Id, StringComparer.OrdinalIgnoreCase);
			...
			{
				string key = up.Id.ToString();
			CategoryGlobalIndexEntry entry = null;
			
			// ✅ STEP 1: Try matching by GUID first (primary match)
			if (!map.TryGetValue(key, out entry))
			{
				// ✅ STEP 2: GUID doesn't match - try matching by MEP+Host+Point (fallback)
				// This handles case where same intersection has different GUIDs in different filters
				// Example: Filter A creates GUID-1, Filter B creates GUID-2 for same intersection
				// We should update the existing entry (GUID-1) instead of creating duplicate (GUID-2)
				if (up.MepElementId > 0 && up.StructuralElementId > 0 &&
				    ...
				    Math.Abs(up.IntersectionPointZ) > 1e-9)
				{
					...
					if (existingEntry != null)
				{
						// ✅ FOUND BY MEP+Host+Point: Update existing entry (reuse GUID from Global XML)
						// This prevents duplicate entries for same intersection
						entry = existingEntry;
						
						if (!DeploymentConfiguration.DeploymentMode)
						{
							DebugLogger.Info($"[GLOBAL-INDEX] ✅ MATCHED BY MEP+Host+Point: Entry {entry.Id} (existing) matches new GUID {key} - updating existing entry instead of creating duplicate");
						}
					}
				}
				
				// ✅ STEP 3: No match found - create new entry with new GUID
				if (entry == null)
				{
					entry = new CategoryGlobalIndexEntry { Id = key, FilterName = filterName ?? string.Empty };
					index.Entries.Add(entry);
					map[key] = entry; // Add to map for next iteration
					changed = true; // New entry created
				}
				}
				
				// Update flags and sleeve IDs
				bool flagsChanged = entry.IsResolved != up.IsResolved || entry.IsClusterResolved != up.IsClusterResolved || entry.SleeveInstanceId != up.SleeveInstanceId || entry.ClusterSleeveInstanceId != up.ClusterSleeveInstanceId;
				if (flagsChanged)
				{
					entry.IsResolved = up.IsResolved;
					entry.IsClusterResolved = up.IsClusterResolved;
				...
				// Update MEP+Host+Point data (always update if provided, even if flags unchanged)
				bool dataChanged = entry.MepElementId != up.MepElementId || entry.StructuralElementId != up.StructuralElementId ||
			...
			// ✅ CRITICAL FIX: Always update FilterName if provided (not just when empty)
			// This ensures FilterName is set correctly when entries are updated
