...
namespace JSE_RevitAddin_MEP_OPENINGS.Commands
{
    ...
    public class PipeSleeveCommand : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            {
                tx.Start();
                // Collect all placed rectangular pipe sleeves for suppression (PipeOpeningOnWallRect)
                var allRectPipeSleeves = new FilteredElementCollector(doc)
                    .OfClass(typeof(FamilyInstance))
                    .Cast<FamilyInstance>()
                    .Where(fi => fi.Symbol?.Family?.Name != null && 
                                fi.Symbol.Family.Name.Contains("PipeOpeningOnWallRect") && 
                                fi.Symbol?.Name != null &&
                                fi.Symbol.Name.Contains("DS#"))
                    .ToList();
                ...
                foreach (var pipe in pipes)
                {
                    ...
                    {
                        // Non-insulated pipe, use default clearance
                    }
                    ...
                    if (isXOrientation)
                    {
                        ...
                        foreach (var group in grouped)
                        {
                            ...
                            if (wallSolid != null) {
                                ...
                                try
                                {
                                    startInside = IsPointInsideSolid(wallSolid, line.GetEndPoint(0), hostWallEntryExit.Orientation);
                                    endInside = IsPointInsideSolid(wallSolid, line.GetEndPoint(1), hostWallEntryExit.Orientation);
                                }
                                catch (Exception ex)
                                {
                                    startInside = false;
                                ...
                                // SUPPRESSION CHECK: Check if pipe sleeve placement point is within any rectangular opening bounding box
                                bool suppressDueToRectOpening = IsPointWithinRectangularOpening(sleevePoint, allRectPipeSleeves, true, hostWallEntryExit);
                                
                                if (suppressDueToRectOpening)
                                {
                                    continue;
                                }
                                ...
                                // No wall solid found, skipping
                            }
                        } // <-- close foreach (var group in grouped)
                        if (!placed) {
                            // No valid X-direction intersection found
...
//                                 DebugLogger.Log($"[PipeSleeveCommand] Skipping very short Y-pipe {pipe.Id.IntegerValue} with length {pipeLineLength:F6}");
                            ...
                            try
                            {
                                foreach (Face face in wallSolid.Faces)
                                {
                                    ...
                                    {
//                                         DebugLogger.Log($"[PipeSleeveCommand] Warning: Face intersection failed for pipe {pipe.Id.IntegerValue}: {ex.Message}");
                                    }
                                }
                            }
                            catch (Exception ex)
                            {
//                                 DebugLogger.Log($"[PipeSleeveCommand] Error: Wall solid face enumeration failed for pipe {pipe.Id.IntegerValue}: {ex.Message}");
                                continue;
                            }
                            bool startInside = false;
                            bool endInside = false;
                            try
                            {
                                startInside = IsPointInsideSolid(wallSolid, pipeLine.GetEndPoint(0), hostWall.Orientation);
                                endInside = IsPointInsideSolid(wallSolid, pipeLine.GetEndPoint(1), hostWall.Orientation);
                            }
                            catch (Exception ex)
                            {
//                                 DebugLogger.Log($"[PipeSleeveCommand] Warning: IsPointInsideSolid check failed for pipe {pipe.Id.IntegerValue}: {ex.Message}");
                                startInside = false;
                                endInside = false;
                            }
                            // Segmented fallback
                            ...
                            // Add null checks before distance calculation
                            if (ptEntry == null || ptExit == null)
                            {
//                                 DebugLogger.Log($"[PipeSleeveCommand] Y-pipe {pipe.Id.IntegerValue}: ptEntry or ptExit is null, skipping");
                                continue;
                            }
...
//                             DebugLogger.Log($"[PipeSleeveCommand] Y-pipe: Placing sleeve in wall {hostWall.Id.IntegerValue} (Type: {wallType}, Function: {wallFunction}), placement method: {placementMethod}");
                            ...
                            // SUPPRESSION CHECK: Check if pipe sleeve placement point is within any rectangular opening bounding box
                            bool suppressDueToRectOpening = IsPointWithinRectangularOpening(sleevePoint, allRectPipeSleeves, false, hostWall);
                            
                            if (suppressDueToRectOpening)
                            {
//                                 DebugLogger.Log($"[PipeSleeveCommand] Y-pipe {pipe.Id.IntegerValue}: Suppressed circular sleeve placement at {sleevePoint} (point is within rectangular opening bounding box)");
                                continue;
                            }
...
//                             DebugLogger.Log($"[PipeSleeveCommand] Y-pipe {pipe.Id.IntegerValue} endpoints: {line.GetEndPoint(0)} to {line.GetEndPoint(1)}");
        ...
        /// <summary>
        /// Checks if a pipe sleeve placement point is within the bounding box of any rectangular cluster sleeve opening
        /// </summary>
        private bool IsPointWithinRectangularOpening(XYZ point, List<FamilyInstance> rectangularOpenings, bool isXPipe = false, Wall hostWall = null)
        {
            try
            {
                foreach (var rectOpening in rectangularOpenings)
                {
                    // Get the opening's location and dimensions from parameters
                    LocationPoint location = rectOpening.Location as LocationPoint;
                    if (location == null) continue;

                    XYZ openingLocation = location.Point;
                    
                    // Get width and height parameters
                    Parameter widthParam = rectOpening.LookupParameter("Width");
                    Parameter heightParam = rectOpening.LookupParameter("Height");
                    
                    if (widthParam == null || heightParam == null)
                    {
                        // Fallback to bounding box method
                        BoundingBoxXYZ boundingBox = rectOpening.get_BoundingBox(null);
                        if (boundingBox == null) continue;
                        
                        double fallbackTolerance = UnitUtils.ConvertToInternalUnits(10.0, UnitTypeId.Millimeters);
                        XYZ min = boundingBox.Min - new XYZ(fallbackTolerance, fallbackTolerance, fallbackTolerance);
                        XYZ max = boundingBox.Max + new XYZ(fallbackTolerance, fallbackTolerance, fallbackTolerance);
                    ...
                    // Determine wall orientation to understand the actual spatial layout
                    bool isYAxisWall = false;
                    if (hostWall != null)
                    {
                        XYZ wallOrientation = hostWall.Orientation;
                        isYAxisWall = Math.Abs(wallOrientation.X) > Math.Abs(wallOrientation.Y);
                    }
                    else
                    {
                        // Fallback: Try to detect wall orientation using ReferenceIntersector near the rectangular opening
                        try
                        {
                            var doc = rectOpening.Document;
                            ElementFilter wallFilter = new ElementCategoryFilter(BuiltInCategory.OST_Walls);
                            
                            // Find a valid 3D view - ReferenceIntersector REQUIRES a 3D view to work properly
                            var view3D = new FilteredElementCollector(doc)
                                .OfClass(typeof(View3D))
                                .Cast<View3D>()
                            ...
                            if (view3D == null)
                            {
                                // Fallback: Use aspect ratio of the opening to guess wall orientation
                                if (width > height * 1.5)
                                {
                                    // Opening is much wider than tall - likely on Y-axis wall (vertical wall)
                                    isYAxisWall = true;
                                }
                                else if (height > width * 1.5)
                                {
                                    // Opening is much taller than wide - likely on X-axis wall (horizontal wall)
                                    isYAxisWall = false;
                                }
                                else
                                {
                                    // Square-ish opening - default to Y-axis wall behavior (most common case)
                                    isYAxisWall = true;
                                }
                            }
                            else
                            {
                                ...
                                {
                                    var references = refIntersector.Find(openingLocation, dir);
                                    if (references != null && references.Count > 0)
                                    {
                                        var refWithContext = references.First();
                                        var refObj = refWithContext.GetReference();
                                        double proximity = refWithContext.Proximity;
                                        
                                        // Only use walls that are very close (within 10cm)
                                        if (proximity > 0.33) // 0.33 feet ≈ 10cm
                                        {
                                            continue;
                                        }
                                        ...
                                        if (refObj.LinkedElementId != ElementId.InvalidElementId)
                                        {
                                            ...
                                            {
                                                detectedWall = linkInstance.GetLinkDocument().GetElement(refObj.LinkedElementId) as Wall;
                                            }
                                        }
                                        else
                                        {
                                            detectedWall = doc.GetElement(refObj.ElementId) as Wall;
                                        }
                                        
                                        if (detectedWall != null)
                                        {
                                            XYZ wallOrientation = detectedWall.Orientation;
                                            isYAxisWall = Math.Abs(wallOrientation.X) > Math.Abs(wallOrientation.Y);
                                            break;
                                        }
                                    }
                                ...
                                if (!isYAxisWall) // If no wall detected via ReferenceIntersector, use aspect ratio fallback
                                {
                                    if (width > height * 1.2)
                                    {
                                        isYAxisWall = true;
                        ...
                        {
                            // Final fallback: Use opening aspect ratio
                            if (width > height * 1.2)
                            {
                                isYAxisWall = true;
                            }
                    ...
                    // For spatial checking, determine the actual dimensions in X and Y space
                    double spatialXDimension, spatialYDimension;
                    if (isYAxisWall)
                    {
                        // Y-axis walls: Width parameter becomes Y-dimension, Height parameter becomes X-dimension
                        spatialXDimension = height;  // Height parameter is used for X-space
                        spatialYDimension = width;   // Width parameter is used for Y-space
                    }
                    else
                    {
                        // X-axis walls: Width parameter is X-dimension, Height parameter is Y-dimension
                        spatialXDimension = width;   // Width parameter is used for X-space
                        spatialYDimension = height;  // Height parameter is used for Y-space
                    }
                    
                    // Calculate the opening's bounding rectangle with tolerance using spatial dimensions
                    double tolerance = UnitUtils.ConvertToInternalUnits(10.0, UnitTypeId.Millimeters); // 10mm tolerance
                    double halfSpatialX = (spatialXDimension / 2.0) + tolerance;
                    double halfSpatialY = (spatialYDimension / 2.0) + tolerance;
                    
                    // Check if point is within the opening's rectangle
                    double deltaX = Math.Abs(point.X - openingLocation.X);
                    double deltaY = Math.Abs(point.Y - openingLocation.Y);
                    double deltaZ = Math.Abs(point.Z - openingLocation.Z);
                    
                    // Use a reasonable Z tolerance for wall thickness
                    double zTolerance = UnitUtils.ConvertToInternalUnits(50.0, UnitTypeId.Millimeters); // 50mm Z tolerance

                    if (deltaX <= halfSpatialX && deltaY <= halfSpatialY && deltaZ <= zTolerance)
                    {
                        return true;
                    }
                }
            ...
            {
                // Error checking rectangular opening bounds - continue without suppression
            }
            return false;
        }
    }
