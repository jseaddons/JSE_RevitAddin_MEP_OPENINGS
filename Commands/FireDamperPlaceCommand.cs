...
namespace JSE_RevitAddin_MEP_OPENINGS.Commands
{
    ...
    public class FireDamperPlaceCommand : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            {
                TaskDialog.Show("Error", "No damper opening family symbol found.");
            ...
            using (var tx = new Transaction(doc, "Place Fire Damper Sleeves"))
            {
                ...
                foreach (var damper in dampers)
                {
                    ...
                    // SUPPRESSION CHECK: Skip dampers that already have sleeves
                    ...
                    // Check if any existing sleeve is within tolerance of this damper
                    var nearbySleeveLocations = existingSleeveLocations.Values.Where(sleeveLocation =>
                        damperLocation.DistanceTo(sleeveLocation) <= sleeveCheckRadius).ToList();

                    if (nearbySleeveLocations.Any())
                    {
                        var closestDistance = nearbySleeveLocations.Min(sleeveLocation => damperLocation.DistanceTo(sleeveLocation));
                        DamperLogger.Log($"Damper ID={damper.Id.IntegerValue}: existing sleeve found within {UnitUtils.ConvertFromInternalUnits(closestDistance, UnitTypeId.Millimeters):F1}mm, skipping");
