    ...
    public class DuctFittingOpeningCommand : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            if (view3D == null)
            {
                ...
                // TaskDialog.Show("Warning", "No 3D view found. Intersection detection may be limited."); // Info dialog removed as requested
            }
            else
            {
            ...
            using (var transaction = new Transaction(doc, "Place Duct Fitting Openings"))
            {
                ...
                if (ductFittings.Count == 0)
                {
                    ...
                    // TaskDialog.Show("Info", "No duct fittings found. Command aborted."); // Info dialog removed as requested
            ...
            if (elementsPlaced == 0)
            {
                // TaskDialog.Show("Info", "No openings placed. See logs for details."); // Info dialog removed as requested
            }
            else
            {
                // TaskDialog.Show("Success", elementsPlaced + " openings placed successfully."); // Info dialog removed as requested
            }
