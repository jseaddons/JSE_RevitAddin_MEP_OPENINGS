    ...
    public class RefreshService
    {
        ...
        public RefreshService(Document document, UIDocument uiDocument, ApplicationProfileService appProfileService)
        {
            ...
            _clashZoneService = new ClashZoneService(new Models.ClashZoneStorage(), msg =>
            {
                if (!DeploymentConfiguration.DeploymentMode)
                    DebugLogger.Info(msg);
            }, null, null); // Explicitly pass null for IFlagManager and GuidManager to avoid ambiguity
        ...
        private Autodesk.Revit.UI.Result ExecuteRefreshInternal(List<string> selectedFilterItems, List<string> selectedMepCategories, List<string> selectedReferenceFiles, List<string> selectedHostFiles, Dictionary<string, double> clearanceSettings)
        {
            ...
            // ✅ SECTION BOX CACHE: Capture and store section box at start of refresh if flag is enabled
            if (OptimizationFlags.UseSectionBoxCache)
            {
                try
                {
                    // ✅ NOTE: GetActive3DView and GetSessionDbConnection don't exist in SectionBoxService
                    // Section box caching logic commented out - would need alternative implementation
                    // var view3D = SectionBoxService.GetActive3DView(_document);
                    // var dbConnection = SectionBoxService.GetSessionDbConnection();
                    // if (view3D != null && dbConnection != null)
                    // {
                    //     SectionBoxService.CaptureAndStore(view3D, dbConnection);
                    //     SafeFileLogger.SafeAppendText("refresh_sectionbox.log", $"[{DateTime.Now}] [SECTIONBOX_CACHE] Section box captured and stored for session.\n");
                    // }
                }
                catch (Exception ex)
                {
                    SafeFileLogger.SafeAppendText("refresh_sectionbox.log", $"[{DateTime.Now}] [SECTIONBOX_CACHE] ERROR: {ex}\n");
                }
            }
            ...
            try
            {
                    ...
                    selectedMepCategories.Count > 0)
                {
                    ...
                    foreach (var category in categoriesToDedupe)
                    {
                        try
                        {
                            ...
                            // ✅ NOTE: GlobalIndexMaintenance doesn't exist
                            // Deduplication logic commented out - would need alternative implementation
                            ...
                            // Since dedupeResult is not available, skip logging dedup results
                            dedupeLogBuilder.AppendLine(
                                $"[{DateTime.Now}] [DEDUPER]   Deduplication skipped (GlobalIndexMaintenance not available)");
                        }
                ...
                if (isReplaceMode)
                {
                    ...
                    else
                    {
                        try
                        {
                            // ✅ NOTE: ResetFlagsForDeletedSleeves method doesn't exist
                            // Flag reset logic commented out - would need alternative implementation
                            // resetCount = _flagManager.ResetFlagsForDeletedSleeves(categoriesForReset, null, refreshLogName);
                            resetCount = 0;
                            SafeFileLogger.SafeAppendText(refreshLogName,
                                $"[{DateTime.Now}] [REPLACE-MODE] Flag reset skipped (method not available). Categories={string.Join(", ", categoriesForReset)}\n");
                        }
                ...
                if (existingClashZones?.ClashZones != null && existingClashZones.ClashZones.Count > 0)
                {
                    ...
                    // 1. Verify existence: Reset flags for deleted sleeves across ALL relevant categories
                    // 2. Set Session context: Mark zones in Section Box + Active Filters as "Context"
                    try
                    {
                        using (var dbContext = new JSE_RevitAddin_MEP_OPENINGS.Data.SleeveDbContext(_document))
                        {
                            var repository = new JSE_RevitAddin_MEP_OPENINGS.Data.Repositories.ClashZoneRepository(dbContext);
                            
                            // A. Hierarchical Reset - O(1) verify existence using HashSet of all Revit IDs
                            int resetCount = repository.VerifyExistingSleevesAndResetFlags(
                                _document, 
                                selectedFilterItems, 
                                selectedMepCategories);
                            
                            if (resetCount > 0 && !DeploymentConfiguration.DeploymentMode)
                                DebugLogger.Info($"[SESSION-CONTEXT] 🛡️ Reset flags for {resetCount} zones with missing sleeves.");

                            // B. Update Session Flags (IsCurrentClash + ReadyForPlacement)
                            // This uses the active Section Box and UI selections to mark what's "Ready"
                            BoundingBoxXYZ sectionBox = null;
                            if (_document.ActiveView is View3D view3D && view3D.IsSectionBoxActive)
                                sectionBox = view3D.GetSectionBox();

                            var sessionSvc = new SessionContextService(repository, msg => { if (!DeploymentConfiguration.DeploymentMode) DebugLogger.Info(msg); });
                            int markedCount = sessionSvc.UpdateSessionFlags(
                                selectedFilterItems, 
                                selectedMepCategories, 
                                sectionBox, 
                            ...
                            if (!DeploymentConfiguration.DeploymentMode)
                                DebugLogger.Info($"[SESSION-CONTEXT] ✅ Marked {markedCount} zones as Ready for Placement.");
                        }
                    }
                    catch (Exception ctxEx)
                    {
                        if (!DeploymentConfiguration.DeploymentMode)
                            DebugLogger.Error($"[SESSION-CONTEXT] ❌ Error updating session context: {ctxEx.Message}");
                    }
                }
                        ...
                        try
                        {
                            if (enableThreePointValidation)
                            {
                                ...
                                if (validationResult.UpdatedIntersectionPoint != null && validationResult.IntersectionPointMovement.HasValue)
                                {
                                    ...
                                    // ✅ PATH 3 (ADOPT): Set ReadyForPlacementFlag=1 if valid but intersection moved
                                    // This tells the placer to replace/update the sleeve at the new location
                                    if (enableThreePointValidation && validationResult.IntersectionPointMovement.Value > 1e-4) // Use a small epsilon
                                    {
                                        existingZone.ReadyForPlacementFlag = true;
                                        if (!DeploymentConfiguration.DeploymentMode)
                                            DebugLogger.Info($"[PATH-3] 📦 Zone {existingZone.Id} marked as Ready for Placement (Intersection Moved Δ={validationResult.IntersectionPointMovement.Value:F3})");
                                    }
                                ...
                                {
                                    // ✅ PATH 1 (SKIP): Sleeve present and clash valid (no movement)
                                    // Ensure ReadyForPlacementFlag is FALSE to prevent redundant placement
                                    existingZone.ReadyForPlacementFlag = false;

                                    if (!DeploymentConfiguration.DeploymentMode)
                                        DebugLogger.Info($"[3-POINT-VALIDATION] ✅ VALID: Zone {existingZone.Id} - All 3 points valid, no update needed (Path 1 Skip)");
                                }
                            }
                            else
                            {
                ...
                else
                {
                    ...
                    activeView3D = _document.ActiveView as View3D;
                    ...
                    // Note: FlagManager doesn't implement IFlagManager interface, pass null
                    _clashZoneService = new ClashZoneService(existingClashZones, msg => batchedLogger.Log(msg), null, _guidManager);
                ...
                if (activeView3D != null && activeView3D.IsSectionBoxActive)
                {
                    try
                    {
                        var secBox = activeView3D.GetSectionBox();
                ...
                if (enabledFilter != null)
                {
                    ...
                    try { targetFilter.SelectedHostCategories = FilterUiStateProvider.GetSelectedHostElementTypes?.Invoke() ?? new List<string>(); } catch { }
                    if (!DeploymentConfiguration.DeploymentMode)
